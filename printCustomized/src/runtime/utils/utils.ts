...
async function getLayerIdByDs(dataSources: UseDataSource[], jimuMapView: JimuMapView, useMapWidgetIds: ImmutableArray<string>, widgetId: string) {
  const isDsInUseMap = checkIsDsInUseMap(dataSources, useMapWidgetIds)
