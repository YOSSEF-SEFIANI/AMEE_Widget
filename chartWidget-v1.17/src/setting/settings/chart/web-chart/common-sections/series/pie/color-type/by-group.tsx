...
`;
export const ByGroupColorList = (
  props: ByGroupColorListProps,
): React.ReactElement => {
  const {
    loading,
  ...
  const handleChange = (
    index: number,
    slice: ImmutableObject<WebChartPieChartSlice>,
  ) => {
    let newSlices = propValue as any;
    if (newSlices.asMutable) {
      newSlices = newSlices.asMutable({ deep: true });
    } else {
      newSlices = [...newSlices];
    }
    newSlices[index] = slice;
    const slices = Immutable(newSlices);
    onChange?.(slices);
  };
  ...
  return (
    <div
      ...
      css={style}
      role="group"
      aria-label={translate("sliceColor")}
    >
      <div className="body">
