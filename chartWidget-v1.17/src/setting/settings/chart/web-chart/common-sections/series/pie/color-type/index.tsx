...
export const ColorType = (props: ColorTypeProps): React.ReactElement => {
  ...
  React.useEffect(() => {
    if (
      ...
      const hasValidSlices =
        propSlices &&
        propSlices.length > 0 &&
        propSlices.some(
          (slice) =>
            slice.label && slice.label !== "null" && slice.label.trim() !== "",
        );

      if (!hasValidSlices) {
        loadSlices(MaxColorCount, outline)
          .then(({ value: slices }) => {
          ...
          .catch((error) => {
            console.error(
              "❌ [ColorType] Failed to auto-reload slices:",
              error,
            );
          });
      }
  ...
  const handleColorTypeChange = (type: "singleColor" | "bySlices") => {
    ...
    if (type === "singleColor") {
      ...
      series = series.map(
        (serie) =>
          (serie as ImmutableObject<WebChartPieChartSeries>).without(
            "slices",
          ) as any,
      );
      onChange?.(series);
      ...
      if (categoryType === CategoryType.ByGroup) {
        ...
        const slices = getByFieldPieSlices(
          numericFields,
          COLORS_SET[0],
          outline,
        );
  ...
  const handleSlicesChange = (
    slices: ImmutableArray<WebChartPieChartSlice>,
  ) => {
  ...
  return (
    <>
      <div
        ...
        aria-label={translate("themeSettingColorMode")}
      >
        <div className="d-flex align-items-center justify-content-between">
          ...
          >
            <Radio
              ...
              onChange={() => {
                handleColorTypeChange("bySlices");
              }}
            />
        ...
        <div className="d-flex align-items-center justify-content-between">
          ...
          >
            <Radio
              ...
              onChange={() => {
                handleColorTypeChange("singleColor");
              }}
            />
