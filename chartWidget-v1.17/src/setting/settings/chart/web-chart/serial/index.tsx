...
import { DualAxisSetting } from "../common-sections/dual-axis";
...
const SerialSetting = (props: SerialSettingProps): React.ReactElement => {
  ...
  const legendValid =
    propWebChart?.series != null && propWebChart?.series?.length > 1;
  ...
  const horizontalAxisLabelsBehavior =
    propWebChart?.horizontalAxisLabelsBehavior;
  const verticalAxisLabelsBehavior = propWebChart?.verticalAxisLabelsBehavior;

  const handleSeriesStatisticsChange = (
    ...
    if (title) {
      webChart = webChart
        .setIn(["title", "content", "text"], title)
  ...
  const handleSeriesChange = (
    series: ImmutableArray<WebChartSeries>,
    seriesRelatedProps?: { valueFormat?: any; colorMatch?: boolean },
  ) => {
    const valueFormat = seriesRelatedProps?.valueFormat;
    const colorMatch = seriesRelatedProps?.colorMatch;
    ...
    <>
      <SettingSection>
        <CollapsablePanel
          ...
          onRequestOpen={() => {
            onSectionChange(ChartSettingSection.Data);
          }}
          onRequestClose={() => {
            onSectionChange(ChartSettingSection.None);
          }}
        >
      ...
      <SettingSection>
        <CollapsablePanel
          ...
          onRequestOpen={() => {
            onSectionChange(ChartSettingSection.Series);
          }}
          onRequestClose={() => {
            onSectionChange(ChartSettingSection.None);
          }}
      ...
      <SettingSection>
        <CollapsablePanel
          ...
          onRequestOpen={() => {
            onSectionChange(ChartSettingSection.Axes);
          }}
          onRequestClose={() => {
            onSectionChange(ChartSettingSection.None);
          }}
          ...
          <SettingRow flow="wrap">
            <AxesSetting
              ...
              onVerticalAxisLabelsBehaviorChange={
                handleVerticalAxisLabelsBehaviorChange
              }
              onHorizontalAxisLabelsBehaviorChange={
                handleHorizontalAxisLabelsBehaviorChange
              }
            />
          </SettingRow>
          <SettingRow flow="wrap">
            <DualAxisSetting
              dualAxisConfig={propWebChart?.dualAxisConfig}
              series={propWebChart?.series}
              axes={propWebChart?.axes}
              onChange={handleDualAxisConfigChange}
      ...
      <SettingSection>
        <CollapsablePanel
          ...
          onRequestOpen={() => {
            onSectionChange(ChartSettingSection.General);
          }}
          onRequestClose={() => {
            onSectionChange(ChartSettingSection.None);
          }}
      ...
      <SettingSection>
        <CollapsablePanel
          ...
          onRequestOpen={() => {
            onSectionChange(ChartSettingSection.Appearance);
          }}
          onRequestClose={() => {
            onSectionChange(ChartSettingSection.None);
          }}
