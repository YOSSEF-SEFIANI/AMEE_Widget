...
export const getDefaultHistogramOverlayColor = (
 ...
 * @returns
 */
export const getNonRepeatingColor = (
  colors: string[],
  index: number = 0,
 ...
 * @returns
 */
export const getNonRepeatingSeriesFillColor = (
  index: number = 0,
  preSerieColor: string,
): string => {
...
export function getDefaultGuide(
  name: string,
  label: string = "",
  ...
  const labelText = getTextSymbol(
    label,
    DefaultGuideLabelSize,
    DefaultTextColor as any,
  );
...
export const getCategoryAxis = (
  text = "",
  showLabels = false,
): WebChartAxis => {
  const title = getChartText(
    text,
    false,
    DefaultAxisTitleSize,
    DefaultAxisTitleColor,
  );
...
/**
 * Créer un deuxième axe Y avec des paramètres par défaut pour la double échelle
 * Note: Le 3ème axe (axes[2]) est automatiquement positionné à droite par ArcGIS Charts
 * quand une série a assignToSecondValueAxis=true
 *
 * IMPORTANT: L'axe secondaire doit avoir une structure complète et identique
 * à l'axe Y primaire pour éviter les erreurs de rendu SVG
 */
export const getDefaultSecondaryAxis = (): WebChartAxis => {
  // Utiliser exactement la même structure que l'axe Y primaire
  const secondaryAxis = getValueAxis("Axe secondaire", true, true);
  return {
    ...secondaryAxis,
    type: "chartAxis", // Explicite pour éviter les problèmes
    visible: true,
    title: {
      ...secondaryAxis.title,
      visible: true,
      content: {
        ...secondaryAxis.title.content,
        // Rotation de 90° pour l'axe Y droit (sens inverse de l'axe gauche)
        angle: -270, // ou 90 pour différencier visuellement
    ...
    // NE PAS définir minimum/maximum - laisser le calcul automatique
    // Le composant Chart calculera basé sur les séries assignées
  } as WebChartAxis;
};
...
export function getGaugeAxis(showLabels: boolean = false): WebChartGaugeAxis {
  let axis = getValueAxis(
    "",
    false,
    showLabels,
    DefaultGaugeAxisLabelSize,
    DefaultGaugeAxisColor,
  ) as WebChartGaugeAxis;

  const innerLabel = getChartText(
    "",
    false,
    DefaultGaugeAxisInnerLabelSize,
    DefaultGaugeAxisInnerLabelColor,
  );
...
export function getDefaultAxes(
  ...
  const defaultGridLine = getLineSymbol(1, DefaultGridColor, "esriSLSDash");
  switch (chartType) {
    case "barSeries": {
 ...
 */
export function getDefaultBarChartSeries(
  index: number = 0,
  colorProps?: SeriesColorProps,
): WebChartSeries {
  ...
  return {
    ...
    dataLabels: getChartText(
      "",
      false,
      DefaultSeriesLabelSize,
 ...
 */
export function getDefaultLineChartSeries(
  index: number = 0,
  colorProps?: SeriesColorProps,
): WebChartSeries {
  const color = !colorProps?.color
...
export function getDefaultScatterPlotChartSeries(): WebChartSeries {
  ...
  return {
    ...
    x: "",
    y: "",
    markerSymbol: getCircleMarkerSymbol(
      color,
      1,
      DefaultMarkerOutlineColor,
      DefaultMarkerSize,
    ),
...
export function getDefaultHistogramOverlays(): HistogramOverlays {
  return {
    ...
    standardDeviation: getDefaultOverlay(
      getDefaultHistogramOverlayColor("standardDeviation"),
    ),
    comparisonDistribution: getDefaultOverlay(
      getDefaultHistogramOverlayColor("comparisonDistribution"),
    ),
  };
}
...
export const getDefaultLegend = (
  ...
  let legend: WebChartLegend = {
    ...
    title: getChartText(
      "",
      true,
      DefaultLegendTitleSize,
      DefaultLegendTitleColor,
    ),
...
export const isSerialSeries = (
  ...
  const seriesType =
    typeof value === "string"
      ? (value as ChartTypes)
      : (getSeriesType(value as any) as string);
 ...
 * @returns {boolean}
 */
export const isXYChart = (
  value: string | WebChartSeries[] | ImmutableArray<WebChartSeries>,
): boolean => {
  if (value == null || (value as string) === "") return false;
  const seriesType =
    typeof value === "string" ? value : (getSeriesType(value as any) as string);
  return (
